...
final class asset_gallery_test extends \advanced_testcase {
     ...
     * @return string The GLB bytes.
     */
    protected function make_glb(array $asset): string {
        $json = json_encode(['asset' => $asset]);
        // Pad the JSON chunk to a 4-byte boundary with spaces, per the GLB spec.
        while (strlen($json) % 4 !== 0) {
            $json .= ' ';
        }
        $jsonchunk = pack('V', strlen($json)) . 'JSON' . $json;
        $total = 12 + strlen($jsonchunk);
        return 'glTF' . pack('V', 2) . pack('V', $total) . $jsonchunk;
    }

    /**
     * Store a GLB with the given asset block in the asset-pack file area.
     *
     * @param string $filename The .glb file name.
     * @param array $asset The glTF asset block.
     */
    protected function make_glb_file(string $filename, array $asset): void {
        get_file_storage()->create_file_from_string([
            'contextid' => \context_system::instance()->id,
            'component' => 'format_mnemo',
            'filearea' => 'assetpack',
    ...
    /**
     * Model cards carry the copyright and generator read from the GLB's glTF
     * asset block: a bundled model exposes its generator, and an uploaded model
     * exposes both fields when present.
     */
    public function test_model_metadata(): void {
        $this->resetAfterTest();
        ...
        // The bundled models are generated by the plugin's own tool.
        $this->assertArrayHasKey('generator', $bykey['av']);
        $this->assertStringContainsString('format_mnemo', (string)$bykey['av']['generator']);
        $this->assertNull($bykey['av']['copyright']);

        // An uploaded model's copyright and generator are surfaced.
        $this->make_glb_file('rocket.glb', [
            'version' => '2.0',
            'generator' => 'Blender 4.2',
            'copyright' => '(c) 2026 Ada Lovelace, CC-BY-4.0',
        ]);
        $bykey = [];
        foreach (asset_gallery::models() as $model) {
            $bykey[$model['key']] = $model;
        }
        $this->assertSame('(c) 2026 Ada Lovelace, CC-BY-4.0', $bykey['rocket']['copyright']);
        $this->assertSame('Blender 4.2', $bykey['rocket']['generator']);
    }
