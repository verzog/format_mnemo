...
- **In-view object editor** — teachers (anyone who can edit the course) get an
  **Edit layout** button on the 3D view. Turn it on, click a building or video
  screen to select it, then scale, move and rotate it with the panel; **Save**
  stores the change per-activity so it renders that way for every learner. Use
  it to enlarge a small imported building or nudge one off a neighbour. Guarded
  by the activity-editing capability, so learners never see it.
...
| `db/services.php` | Declares the `format_mnemo_set_transform` external function. |
