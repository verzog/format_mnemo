...
- **Ring texture URL** / **Upload ring texture** — skin the ringed planets with
  ...
  bands (transparency shows the gaps). URL or upload (the URL wins). **Leave both
  blank** to keep a plain banded ring. Only used in the Void.

All uploaded textures — road, ground, sidewalk, planet, ring and sky — are
downscaled to a bounded size before reaching the GPU (like the sign texture).
The road, ground and sidewalk textures tile best with a **seamless / tileable**
...
Mnemo is a **Moodle course-format plugin**, so the "framework" is Moodle
itself on the server and **Three.js** in the browser. There is no separate
web-app framework — no React, Vue, Node service or bundler beyond Moodle's own
...
- **Output / renderer API + Mustache** — `classes/output/renderer.php` and
  `templates/scene.mustache`; `classes/output/scene.php` builds the
  section/activity graph as JSON for the client.
- **External (Web Services) API** — `classes/external/*` for the in-view
  editor, object placer and content/state endpoints, declared in
  `db/services.php` and guarded by course capabilities.
- **Admin settings API** — `settings.php` (`admin_setting_*`) plus the
  `admin_category`/`admin_externalpage` that host the settings and the
  read-only **Asset viewer** (`preview.php`, `classes/output/asset_gallery.php`).
- **File API** — `format_mnemo_pluginfile` serves uploaded asset packs and
  textures from the system context via `stored_file` / pluginfile URLs.
- **MUC caching** (`db/caches.php`), **XMLDB** schema + `db/upgrade.php`,
  **backup/restore** (`backup/moodle2/`), the **Privacy API** null provider,
  **events/observers**, and standard **lang strings**.

**Client — vanilla JS, no UI framework**

- **Three.js `0.160.1`** (bundled, MIT) loaded as a same-origin ES module, with
  its **addon modules** — `GLTFLoader` and the **Draco**, **KTX2/Basis** and
  **meshopt** decoders (`thirdparty/jsm/`) — for compressed asset packs.
- **WebXR** for the headset path; plain WebGL (with a hand-rolled bloom/tone-map
  pipeline) for the on-screen fallback.
- A self-contained **built-in glTF/GLB parser** in `vr.js` as a fallback for
  when the addon loaders can't load (e.g. a strict CSP).
- Shipped as one **AMD/RequireJS** module (`amd/src/vr.js`), authored in a
  Moodle-lint-clean ES5 style (no nested ternaries, no empty functions, etc.).

**Build & tooling**

- **Grunt + Rollup** (Moodle's `grunt amd`) builds `amd/build/vr.min.js` (+ source
  map); **ESLint** and **Stylelint** run via grunt.
- The bundled prop models are generated by a small Node script
  (`tools/gen-models.mjs`).

**Testing & CI**

- **PHPUnit** (`advanced_testcase`) for the server code (`tests/`).
- A bespoke **Playwright + headless Chromium** "WebXR gesture smoke test"
  (`tests/webxr/smoke.mjs`, Node ESM) drives the client module's logic
