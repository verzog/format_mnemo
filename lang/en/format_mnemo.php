...
$string['setting_planettextures_desc'] = 'Upload up to nine equirectangular (2:1 lat-long) images to use as the surfaces of the planets in the Void environment. Each image is applied to one planet, in filename order. Include the word "ring" in a file\'s name (for example "saturn-ring.png") to give that planet a ring. Leave empty to keep the procedural planets. Only used in the Void environment.';
...
$string['setting_ringtextureurl_desc'] = 'URL of a ring image for the Void\'s ringed planets: a radial strip read from the inner edge (left) to the outer edge (right) and wrapped once around the ring, so it reads as concentric bands. A planet is ringed when its uploaded planet texture\'s filename contains the word "ring". Leave this blank to upload an image below or to keep the plain banded ring. If set, this URL takes precedence over any uploaded image. A hosted image must allow cross-origin use (its server needs a permissive CORS policy), and your site\'s content security policy must allow the image source.';
